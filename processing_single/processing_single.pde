/*
  ...
  La interfaz también acepta mensajes RF1 y RF2 de versiones
  anteriores, pero los presenta como mediciones de un único detector.
...
/*
  false es la opción más segura cuando el archivo también
  será compartido públicamente.
*/
...
/*
  ...
  El límite de 5 % no es un umbral físico.
  Es solo el límite superior mínimo del gráfico.
*/
float rfDisplayMaximum = 5;
...
color rfColor = #31B7B4;
color rfSecondaryColor = #708FFF;
...
void setup() {
  ...
  surface.setTitle(
    "Scanner BLE y detector RF con un nRF24L01"
  );

  /*
    Se bloquea el redimensionamiento para evitar bloqueos
    producidos por dimensiones demasiado pequeñas.
  */
...
void refreshSerialPorts() {
  ...
  if (
    ...
    statusMessage =
      "No se detectaron puertos; revise el USB";
  } else {
    statusMessage =
      serialPorts.length +
      " puerto(s) detectado(s)";
  }
}
...
void csvFileSelected(
  ...
  try {
    ...
    /*
      Se conservan estos encabezados para mantener compatibilidad
...
void drawSingleRadioBadge(
  ...
  text(
    "1 × nRF24L01 | VSPI | barrido secuencial",
    x + w / 2,
    y + h / 2
  );
}
...
void drawRFHistoryPanel(
  ...
  text(
    "Cada fila es un barrido; el más reciente aparece abajo",
    panelX + 224,
    panelY + 10
  );
...
void drawRSSIPanel(
  ...
  text(
    "RSSI informado por el receptor BLE del ESP32",
    panelX +
    panelW -
    14,
    panelY + 11
...
void parseSerialMessage(
  ...
  try {
    /*
      RF es el formato de la versión con un único nRF24L01.

      RF1 y RF2 se mantienen por compatibilidad con archivos
      o firmware anteriores.
    */
    if (
      (
