...
This repository contains a compact 2.4 GHz monitoring platform based on an ESP32 and external nRF24L01 transceivers. The project supports both single-radio
and dual-radio RF scanning configurations.
...
2. **Relative RF-energy detection**, performed by one or two external nRF24L01 receivers while scanning frequencies from 2402 MHz to 2480 MHz.
...
The dual-radio version uses:

```text
RF1,frequency_MHz,filtered_detection_pct,raw_detection_pct
RF2,frequency_MHz,filtered_detection_pct,raw_detection_pct
```
Where:

* RF1 identifies the nRF24L01 connected through VSPI.
* RF2 identifies the nRF24L01 connected through HSPI.

Examples:

```text
RF1,2426,7.84,8.33
RF2,2462,12.45,12.50
```
...
The `radio` column identifies the RF receiver that generated each measurement:

| Configuration | `radio` value | Receiver |
|---|---|---|
| Single | `nRF24L01` | Single sequential RF receiver |
| Dual | `VSPI` | nRF24L01 #1, channels 2–41 |
| Dual | `HSPI` | nRF24L01 #2, channels 42–80 |

BLE rows leave this column empty because BLE measurements are generated by
the integrated ESP32 Bluetooth subsystem rather than by an nRF24L01 receiver.
...
2. Open the interface corresponding to the hardware configuration:

   **Single nRF24L01 version**

   ```text
   processing_single/processing_single.pde
    ```
   **Dual nRF24L01 version**

   ```text
   processing_dual/processing_dual.pde
    ```
...
7. Press `CONECTAR`.
8. Verify that RF measurements begin to appear.
9. Wait for the ESP32 BLE scan to report nearby devices.
10. Press `REGISTRAR CSV` to start recording.
11. Select the destination filename.
12. Press `GUARDAR CSV` to close the recording correctly.
...
```bash
python  python/analyze_ble_rf24_single_csv.py
```
or:

```bash
python python/analyze_ble_rf24_dual_csv.py
```
...
```matlab
analyze_ble_rf24_single_csv
```
or:
```matlab
analyze_ble_rf24_dual_csv
```
...
* The single version scans all frequencies sequentially with one nRF24L01.
* The dual version observes two frequencies simultaneously during each
  measurement window, but each receiver still advances sequentially through
  its assigned frequency range.
